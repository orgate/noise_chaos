  ...
  DOUBLE PRECISION, ALLOCATABLE :: YM1(:),YM2(:),ZM1(:),ZM2(:),C1(:),C2(:),H(:) 
  INTEGER N,K,L,IW,K1,K2,K3,CC,NP,MCH,MP,MF,MC,K4,MMCH,MMP,MMF,MATKEY,IC,KC,NH
  INTEGER, ALLOCATABLE :: indx(:)
  DOUBLE PRECISION T, DT, TFIN, TMEAS, TM, DIF,ran3,CH,LX,LT,LEXP,TEXP,EXPAV, NORM,LH,LHMAX
  ...
  ALLOCATE(X(N),DX(N),XN(N),WR(N),WI(N),H(-NH:NH))
  ...
  MMCH=0 ! NUMBER OF CHAOTIC TRAJECTORIES FOR EIGEN VALUE
  MMP=0  ! NUMBER OF PERIODIC TRAJECTORIES FOR EIGEN VALUE
  MMF=0  ! NUMBER OF FIXED POINT TRAJECTORIES FOR EIGEN VALUE
...
44     XP=(RAN3(IW)*2-1.D0)*4*GW ! GAUSSIAN ASSIGNMENT
  ...
  T=0.D0
  H=0.D0
  DO WHILE (T.LE.TFIN)!.AND.DIF.LE.EPS)
...
!!$  OPEN(UNIT=10, FILE='xymax_st.dat')
!!$  DO K1=1,K4
!!$     WRITE(10,*)Y1(K1),Y2(K1)
!!$  END DO
!!$  CLOSE(10)
!!$ OPEN(UNIT=15, FILE='xymin_st.dat')
!!$  DO K1=1,K4
!!$     WRITE(15,*)Z1(K1),Z2(K1)
!!$  END DO
!!$  CLOSE(15)
!!$ OPEN(UNIT=13, FILE='xycycle_st.dat')
!!$  DO K1=1,K4
!!$     WRITE(13,*)C1(K1),C2(K1)
!!$  END DO
!!$  CLOSE(13)
...
!!$  OPEN(UNIT=25, FILE='xymax_ma.dat')
!!$  DO K1=1,K4
!!$     WRITE(25,*)YM1(K1),YM2(K1)
!!$  END DO
!!$  CLOSE(25)
!!$ OPEN(UNIT=35, FILE='xymin_ma.dat')
!!$  DO K1=1,K4
