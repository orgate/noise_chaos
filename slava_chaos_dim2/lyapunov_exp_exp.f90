  ...
  DOUBLE PRECISION, ALLOCATABLE :: YM1(:),YM2(:),ZM1(:),ZM2(:),C1(:),C2(:),H(:), HX(:), HXI(:) ! HXI(:) added by Alfred 
...
54     XP=(RAN3(IW)*2-1.D0)*4*GW ! GAUSSIAN ASSIGNMENT 
       IF(GS(XP,GW).LE.RAN3(IW)) GO TO 54 !this provides a weight on the random number such that it comes from a gaussian probablity distribution  
         A(K1,K2,K3)=XP
        END DO
...
77   DIF=0.D0
     ...
     ! Newly added by Alfred for plotting histogram of 1/X
!            DO K=1,0
!            K2 = K2
!        K2=(NX/X(K))/2
!        IF (IABS(K2).LE.NX) HXI(K2)=HXI(K2)+1 !HISTOGRAM OF THE X. 
!     END DO
...
!  OPEN(UNIT=120, FILE='histxi3.dat.txt') ! writing the current inverse trajectory (Alfred)
!  DO K1=-NX,NX
!     WRITE(120,*)K1*XMAX/NX,HXI(K1)
!  END DO
!  CLOSE(120)
...
! Y=Y_i, DY= Y'_i, n-dim
      INTEGER n,k1,k2,k3, iw ! added "iw" by Alfred
      DOUBLE PRECISION y(n),dy(n),a(n,n,n),b(n,n)
      DOUBLE PRECISION S,R,BE,AA
      AA=1.D0
      dy=0.d0
      do k1=1,n
         dy(k1)=-y(k1)**3! + 1*y(k1)*rand()    ! added ran3 by Alfred
!         dy(k1)=-y(k1)*dabs(y(k1))*n*n
          do k2=1,n
            dy(k1)=dy(k1) + b(k1,k2)*y(k2) 
            do k3=1,n
!            dy(k1)=dy(k1) + a(k1,k2,k3)*y(k2)*y(k3)  
            dy(k1)=dy(k1) + (a(k1,k2,k3)+0.0001*rand())*y(k2)*y(k3)  ! added ran3 by Alfred
            end do   
            end do
            end do
